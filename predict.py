...
# 保存预测图像
def save_nifti(pred, save_path, affine=np.eye(4)):
    """
    保存预测结果为 NIfTI (.nii.gz) 格式
    Args:
        pred: 3D 预测结果张量，通常形状为 (H, W, T)
        save_path: 保存的文件路径
        affine: 图像的仿射矩阵，一般使用单位矩阵
    """
    # 将预测张量转换为合适的数据类型（例如，uint8）
    pred_np = pred.cpu().numpy().astype(np.uint8)  # 更改为 uint8 或 int16 根据需要
    nifti_img = nib.Nifti1Image(pred_np, affine)  # 创建 NIfTI 图像对象
    nib.save(nifti_img, save_path)  # 保存为 .nii.gz 文件
...
def test_softmax(
    ...
    for i, data in enumerate(test_loader):
        ...
        #   # 在 save_dir 中加入组号 zuhao，构建每个组独立的目录
        #   group_save_dir = os.path.join(save_dir, f"group_{zuhao}")
        #   # 创建保存目录（如果不存在）
        #   os.makedirs(group_save_dir, exist_ok=True)
        #   for k, name in enumerate(names):
        #       # 使用 group_save_dir 作为保存路径
        #       save_path = os.path.join(group_save_dir, f"{name}_prediction.nii")
        #       save_nifti(pred[k], save_path)  # 保存为 NIfTI 文件
    ...
    print(msg)
    model.train()
    return vals_evaluation.avg, pred
