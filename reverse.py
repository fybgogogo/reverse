    ...
    def __init__(self, embedding_dim, depth, heads, mlp_dim, dropout_rate=0.1, n_levels=1, n_points=4):
        ...
        for j in range(self.depth):
            self.cross_attention_list.append(
                Residual(
                    PreNormDrop(embedding_dim, dropout_rate,
...
def create_feed_forward(dim, dropout_rate=0.1):
...
class RFIM(nn.Module):
    def __init__(self, dim):
        ...
        self.feed_forward_s1 = create_feed_forward(dim)
        self.feed_forward_s2 = create_feed_forward(dim)
        self.feed_forward_t1 = create_feed_forward(dim)
        self.feed_forward_t2 = create_feed_forward(dim)

    def forward(self, x):
        # 正向
        half_sequence_length = x.shape[1] // 2
        forward_part1 = x[:, :half_sequence_length, :]
        forward_part2 = x[:, half_sequence_length:, :]

        forward_s_out1 = self.feed_forward_s1(forward_part1)
        forward_t_out1 = self.feed_forward_t1(forward_part2)

        # 计算正向输出
        forward_out = torch.cat((
            self.feed_forward_t2(forward_s_out1 + forward_part2),
            self.feed_forward_s2(forward_s_out1 + forward_t_out1 + forward_part2)
        ), dim=1)

        # 反向
        inverse_part1 = forward_out[:, :half_sequence_length, :]
        inverse_part2 = forward_out[:, half_sequence_length:, :]
        ...
        return forward_out, out

class RFIM_half(nn.Module):
    def __init__(self, dim):
        super().__init__()
...
def channel_shuffle(x, groups):
    batchsize, num_channels, depth, height, width = x.data.size()  # 3D MRI size
...
class DSAM(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, activation='relu'):
        ...
        self.channel_att = nn.Sequential(
            ...
            nn.Conv3d(in_channels, inter_channels, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm3d(inter_channels),
            nn.ReLU(inplace=True),
            nn.Conv3d(inter_channels, in_channels, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm3d(in_channels),
        )
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.up_dwc = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='trilinear', align_corners=False),  # 3D upsampling
            nn.Conv3d(self.in_channels, self.in_channels, kernel_size=kernel_size, stride=stride,padding=kernel_size // 2, groups=self.in_channels, bias=False),
            nn.BatchNorm3d(self.in_channels),
            nn.ReLU(True)
        )
        self.pwc = nn.Conv3d(self.in_channels, self.out_channels * 2, kernel_size=1, stride=1, padding=0, bias=True)
        self.spatialAttention = Spatial_Attention()
    def forward(self, x):
...
class Model(nn.Module):
    def __init__(self, num_cls=4):
        ...
        self.is_training = True #训练的时候使用True

        for m in self.modules():
            if isinstance(m, nn.Conv3d):
                torch.nn.init.kaiming_normal_(m.weight)

        self.reverse_flair = RFIM(dim=transformer_basic_dims)
        self.reverse_t1 = RFIM(dim=transformer_basic_dims)
        self.reverse_t1ce = RFIM(dim=transformer_basic_dims)
        self.reverse_t2 = RFIM(dim=transformer_basic_dims)
        self.reverseBlock_half_flair=RFIM_half(dim=transformer_basic_dims)
        self.reverseBlock_half_t1=RFIM_half(dim=transformer_basic_dims)
        self.reverseBlock_half_t1ce=RFIM_half(dim=transformer_basic_dims)
        self.reverseBlock_half_t2=RFIM_half(dim=transformer_basic_dims)
    ...
    def forward(self, x, mask):
        ...
        intra_token_x5 = [flair_intra_token_x5, t1ce_intra_token_x5, t1_intra_token_x5, t2_intra_token_x5]

        if self.is_training:
            flair_reverse_forward_out, flair_inverse_out = self.reverse_flair(intra_token_x5[global_modality])
            t1_reverse_forward_out, t1_inverse_out = self.reverse_t1(intra_token_x5[global_modality])
            t1ce_reverse_forward_out, t1ce_inverse_out = self.reverse_t1ce(intra_token_x5[global_modality])
            t2_reverse_forward_out, t2_inverse_out = self.reverse_t2(intra_token_x5[global_modality])
            reverse_forward_out=[flair_reverse_forward_out, t1_reverse_forward_out, t1ce_reverse_forward_out, t2_reverse_forward_out]
            ...
            t2_intra_x5 = t2_intra_token_x5.view(x.size(0), patch_size, patch_size, patch_size,
                                                transformer_basic_dims).permute(0, 4, 1, 2, 3).contiguous()
            if mask[0][0] is False:
                flair_intra_x5 = flair_reverse_forward_out.view(x.size(0), patch_size, patch_size, patch_size, transformer_basic_dims).permute(0, 4, 1, 2, 3).contiguous()
            if mask[0][1] is False:
                t1_intra_x5 = t1_reverse_forward_out.view(x.size(0), patch_size, patch_size, patch_size, transformer_basic_dims).permute(0, 4, 1, 2, 3).contiguous()
            if mask[0][2] is False:
                t1ce_intra_x5 = t1ce_reverse_forward_out.view(x.size(0), patch_size, patch_size, patch_size, transformer_basic_dims).permute(0, 4, 1, 2, 3).contiguous()
            if mask[0][3] is False:
                t2_intra_x5 = t2_reverse_forward_out.view(x.size(0), patch_size, patch_size, patch_size, transformer_basic_dims).permute(0, 4, 1, 2, 3).contiguous()

        if not self.is_training:
            flair_inverse_out = flair_intra_token_x5
            t1_inverse_out = t1_intra_token_x5
            t1ce_inverse_out = t1ce_intra_token_x5
            t2_inverse_out = t2_intra_token_x5

            if mask[0][0] is False:
                flair_intra_token_x5 = intra_token_x5[global_modality]
                flair_inverse_out=self.reverseBlock_half_flair(flair_intra_token_x5)
            if mask[0][1] is False:
                t1_intra_token_x5 = intra_token_x5[global_modality]
                t1_inverse_out = self.reverseBlock_half_t1(t1_intra_token_x5)
            if mask[0][2] is False:
                t1ce_intra_token_x5 = intra_token_x5[global_modality]
                t1ce_inverse_out = self.reverseBlock_half_t1ce(t1ce_intra_token_x5)
            if mask[0][3] is False:
                t2_intra_token_x5 = intra_token_x5[global_modality]
                t2_inverse_out = self.reverseBlock_half_t2(t2_intra_token_x5)

            flair_intra_x5 = flair_inverse_out.view(x.size(0), patch_size, patch_size, patch_size,
                                                    transformer_basic_dims).permute(0, 4, 1, 2, 3).contiguous()
            t1ce_intra_x5 = t1_inverse_out.view(x.size(0), patch_size, patch_size, patch_size,
                                                transformer_basic_dims).permute(0, 4, 1, 2, 3).contiguous()
            t1_intra_x5 = t1ce_inverse_out.view(x.size(0), patch_size, patch_size, patch_size,
                                                transformer_basic_dims).permute(0, 4, 1, 2, 3).contiguous()
            t2_intra_x5 = t2_inverse_out.view(x.size(0), patch_size, patch_size, patch_size,
                                              transformer_basic_dims).permute(0, 4, 1, 2, 3).contiguous()
        inverse_out = [flair_inverse_out, t1_inverse_out, t1ce_inverse_out, t2_inverse_out]
        if self.is_training:
        ...
        x1 = self.masker(torch.stack((flair_x1, t1ce_x1, t1_x1, t2_x1), dim=1), mask)  #Bx4xCxHWZ (1,32,128,128,128)
        x2 = self.masker(torch.stack((flair_x2, t1ce_x2, t1_x2, t2_x2), dim=1), mask)  #(1,64,64,64,64)
        x3 = self.masker(torch.stack((flair_x3, t1ce_x3, t1_x3, t2_x3), dim=1), mask)  #(1,128,32,32,32)
        x4 = self.masker(torch.stack((flair_x4, t1ce_x4, t1_x4, t2_x4), dim=1), mask)  #(1,256,16,16,16)
        #x5_intra = self.masker(torch.stack((flair_intra_x5, t1ce_intra_x5, t1_intra_x5, t2_intra_x5), dim=1),
        #                       mask)  #(1,2048,8,8,8)
        x5_intra = torch.stack((flair_intra_x5, t1ce_intra_x5, t1_intra_x5, t2_intra_x5), dim=1).reshape(1, 2048, 8, 8, 8)# (1,2048,8,8,8)


        flair_intra_x5, t1ce_intra_x5, t1_intra_x5, t2_intra_x5 = torch.chunk(x5_intra, num_modals,
                                                                              dim=1)  # （1，512，8，8，8）
        ...
        multimodal_pos = torch.cat((self.flair_pos, self.t1ce_pos, self.t1_pos, self.t2_pos), dim=1)  #（1，2048，512）
        multimodal_inter_token_x5 = self.multimodal_transformer(multimodal_token_x5, multimodal_pos)  #（1，2048，512）3
        multimodal_inter_x5 = self.multimodal_decode_conv(
            multimodal_inter_token_x5.view(multimodal_inter_token_x5.size(0), patch_size, patch_size, patch_size,
                                           transformer_basic_dims * num_modals).permute(0, 4, 1, 2,
                                                                                        3).contiguous())  #（1，2048，8，8，8）
        x5_inter = multimodal_inter_x5  # （1，2048，8，8，8）

        fuse_pred, preds = self.decoder_fuse(x1, x2, x3, x4, x5_inter)  # fuse_pred(1,4,128,128,128)


        if self.is_training:
            return (fuse_pred, (flair_pred, t1ce_pred, t1_pred, t2_pred), preds, intra_token_x5[global_modality], reverse_forward_out,
                    inverse_out, intra_token_x5)
